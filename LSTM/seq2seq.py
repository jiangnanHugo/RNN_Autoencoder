...
#input data X contains word embedding, so X=fmatrix() (x denotes one input sentence)
# Besides, Y is also word embedding , so we dont't need to build word_vocabulary.
...
class LSTMDecoder(LSTM):
    ...
    def decode(self, h_tm1, c_tm1, timesteps):
        ...
        return T.flatten(outputs[0], 2), updates     # outputs[0]:y_1,y_2...,y_n
...
class Seq2Seq(object):
    ...
    def compile(self, loss_func, optimizer):
        ...
        decode_timesteps = T.iscalar('decode_steps')
        ...
        self._predict = theano.function([seq_input, decode_timesteps],
                                        seq_predict_flex,
                                        updates=updates_encode+updates_decode_flex)
        self._test = theano.function([seq_input, seq_target],
                                     loss,
                                     updates=updates_encode+updates_decode)

        updates = []
        updates += updates_encode
        updates += updates_decode
        updates += optimizer(self.params, loss)
        self._train = theano.function([seq_input, seq_target],
                                      loss,
                                      updates=updates)
